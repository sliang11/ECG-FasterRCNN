...
__global__ void RoIPoolFForward(const int nthreads, const T* bottom_data,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    //int maxidx = -1;
    int maxidx = 0;
    const T* offset_bottom_data =bottom_data + (roi_batch_ind * channels + c)* width;
...
__global__ void RoIPoolFBackward(const int nthreads, const T* top_diff,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    int bottom_offset =  (roi_batch_ind * channels + c) * width;
...
at::Tensor ROIPool_backward_cuda(const at::Tensor& grad,
  ...
  auto num_rois = rois.size(0);
  auto grad_input = at::zeros({batch_size, channels, width}, grad.options());
